...
static bool isBoundary(char32_t c)
{
	...
	std::array ranges{
		Range{0x20, 0x2F}, Range{0x3A, 0x40}, Range{0x5B, 0x60}, Range{0x7B, 0xBF}};
...
static float substringSizeX(std::string_view s,
							std::size_t start,
							std::size_t length = std::string_view::npos)
{
	...
	// Dereferencing std::string_view::end() may be undefined behavior in some
	// compilers, because of that, we need to get the pointer value manually if
	// stringEnd == s.end().
...
static std::size_t
getCharIndex(std::string_view s, float cursorPosX, std::size_t start, std::size_t end)
{
	...
	// Display width of the entire string up to the midpoint, gives the
	// x-position where the (midIdx
	// + 1)th char starts
	float widthToMid = substringSizeX(s, 0, midIdx + 1);

	// Same as above but exclusive, gives the x-position where the (midIdx)th
	// char starts
...
TextSelect::Selection TextSelect::getSelection() const
{
	...
	bool startBeforeEnd = selectStart.y < selectEnd.y ||
...
static ImVector<std::string_view>
wrapText(std::string_view text, float wrapWidth, ImFont *font)
{
	...
	while (wrappedLineEnd != textEnd)
	{
		...
		wrappedLineEnd =
			font->CalcWordWrapPositionA(1, wrappedLineStart, textEnd, wrapWidth);

		if (wrappedLineEnd - wrappedLineStart != 0)
		{
			result.push_back(
				{wrappedLineStart,
				 static_cast<std::size_t>(wrappedLineEnd - wrappedLineStart)});
		}
...
void TextSelect::handleMouseDown(const ImVector<TextSelect::SubLine> &subLines,
								 const ImVec2 &cursorPosStart)
{
	if (subLines.size() == 0)
	{
		return;
	...
	std::size_t charsInWholeLineBeforeSubLine = utf8Length(
		{currentWholeLine.data(),
		 static_cast<std::size_t>(currentSubLine.data() - currentWholeLine.data())});

	std::size_t wholeX =
		getCharIndex(currentSubLine, mousePos.x) + charsInWholeLineBeforeSubLine;
		...
		mouseClicks > 0)
	{
		if (mouseClicks % 3 == 0)
		{
			...
			while (firstSubLineIndex > 0 &&
				   subLines[firstSubLineIndex - 1].wholeLineIndex ==
					   subLines[firstSubLineIndex].wholeLineIndex)
			{
				--firstSubLineIndex;
			}
			...
			while (lastSubLineIndex < subLines.size() - 1 &&
				   subLines[lastSubLineIndex + 1].wholeLineIndex ==
					   subLines[lastSubLineIndex].wholeLineIndex)
			{
				++lastSubLineIndex;
			}
			...
			selectEnd = {atLastLine ? utf8Length(currentWholeLine) : 0,
						 atLastLine ? wholeY : wholeY + 1};
...
static void drawSelectionRect(
	const ImVec2 &cursorPosStart, float minX, float minY, float maxX, float maxY)
{
...
void TextSelect::drawSelection(const ImVector<TextSelect::SubLine> &subLines,
							   const ImVec2 &cursorPosStart) const
{
	if (!hasSelection())
	{
		return;
	...
	for (auto subLine : subLines)
	{
		...
		if (startY > subLine.wholeLineIndex ||
			subLine.wholeLineIndex == startY && startX >= subLineEndX)
		{
			continue;
		}
		// Skip whole/sub lines after selection.
		if (endY < subLine.wholeLineIndex ||
			subLine.wholeLineIndex == endY && endX < subLineStartX)
		{
			break;
		}

		// The first and last rectangles should only extend to the selection
		// boundaries The middle rectangles (if any) enclose the entire line +
		// some extra width for the newline.
		bool isStartSubLine = subLine.wholeLineIndex == startY &&
							  subLineStartX <= startX && startX <= subLineEndX;
		bool isEndSubLine = subLine.wholeLineIndex == endY && subLineStartX <= endX &&
							endX <= subLineEndX;

		float minX = isStartSubLine
						 ? substringSizeX(subLine.string,
										  0,
										  startX - std::min(subLineStartX, startX))
						 : 0;
		float maxX =
			isEndSubLine
				? substringSizeX(subLine.string, 0, endX - std::min(subLineStartX, endX))
...
void TextSelect::copy() const
{
	...
	for (std::size_t i = startY; i <= endY; i++)
	{
		...
		std::string_view lineToAdd =
			line.substr(stringStart - line.begin(), stringEnd - stringStart);
		selectedText += lineToAdd;

		// If lines before the last line don't already end with newlines, add
		// them in
...
void TextSelect::update()
{
	...
	// Set block_input when there's an active selection to prevent editor
	// shortcuts from interfering Only set to true when there's a selection,
	// don't set to false to avoid overriding other systems
	if (hasSelection())
	{
